"""FastAPI wrapper around the existing GIDS disaster evacuation pipeline.

Existing dashboard API routes remain unchanged. The /api/mobile/* router is an
additive Phase 2 layer that stores mobile-only data in PostgreSQL and reads
completed scenario JSON files without changing Modules 1-6.
"""
...
from mobile.notification_service import create_completed_scenario_notification
...
# Existing local behavior remains the default: backend/scenarios.
# Railway may set SCENARIOS_DIR=/app/scenarios to make the mounted volume
# explicit. This changes only where completed scenario JSON is stored/read;
# it never changes the pipeline algorithms or Dataset1.xlsx paths.
SCENARIOS_DIR = Path(
    os.getenv("SCENARIOS_DIR", str(BACKEND_ROOT / "scenarios"))
).expanduser().resolve()
...
# ----------------------------------------------------------------------
# REQUEST / RESPONSE MODELS (existing dashboard API)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# PIPELINE EXECUTION (existing implementation)
# ----------------------------------------------------------------------

_pipeline_lock = threading.Lock()


def _run_new_scenario(job_id: str, payload: ScenarioRequest) -> None:
    try:
        ...
        scenario_id = Path(scenario_file).stem
        # The pipeline has successfully completed Modules 1–6 at this point.
        # Notification creation is explicitly best-effort: a PostgreSQL/inbox
        # failure must never turn a completed evacuation analysis into FAILED.
        try:
            create_completed_scenario_notification(scenario_file)
        except Exception:
            logger.exception(
                "Pipeline completed, but automatic scenario notification creation failed."
            )
...
def _run_existing_scenario(job_id: str, scenario_id: str) -> None:
    try:
...
async def lifespan(_: FastAPI):
    """Close only the additive mobile Postgres pool during shutdown."""
    yield
    close_pool()
...
app = FastAPI(
    ...
    description=(
        ...
        "and provides additive mobile destination-selection endpoints."
    ),
    version="1.1.0",
    lifespan=lifespan,
)

# Expose the already-resolved directory to mobile services. Both the existing
# dashboard routes and new mobile routes now read exactly the same scenario
# files and Railway mounted volume.
...
# Additive only. This creates /api/mobile/* and does not override existing API
# routes below because the prefixes do not overlap.
app.include_router(mobile_router)


# ----------------------------------------------------------------------
# EXISTING DASHBOARD ENDPOINTS — preserved route paths and behavior
# ----------------------------------------------------------------------
...
def rerun_scenario(payload: RerunRequest):
    ...
    thread = threading.Thread(
        target=_run_existing_scenario,
        args=(job_id, payload.scenario_id),
        daemon=True,
    )
    thread.start()
...
def list_scenarios():
    ...
    for file_path in files:
        try:
            ...
            summary = module6.get("final_recommendation", {}).get(
                "scenario_summary", {}
            )
            summaries.append(
                {
                    ...
                    "accommodation_percent": summary.get(
                        "PopulationAccommodationPercent"
                    ),
                }
            )
        except Exception:  # noqa: BLE001
            # Preserve existing behavior: one corrupt/incomplete JSON file
            # cannot make scenario history unavailable.
            continue
